...
def init_db():
    with get_db() as conn:
        ...
        # Migrations to add profile fields to users table if they do not exist
...
def reverse_geocode(lat, lon):
    ...
    # Realistic fallback database based on Bengaluru coordinates (lat: 12.95 - 13.05, lon: 77.55 - 77.65)
    fallbacks = [
    ...
    # Deterministically select index based on lat and lon values
...
def add_or_merge_alert(alert):
    ...
    with lock:
        ...
        for existing in state["alerts"]:
            e_gps = existing.get("gps") or existing.get("gps_location")
            if e_gps and "latitude" in e_gps and "longitude" in e_gps:
                if abs(e_gps["latitude"] - lat) < 0.0005 and abs(e_gps["longitude"] - lon) < 0.0005:
                    ...
                    if "gps" in existing:
                        existing["gps"] = e_gps
                    if "gps_location" in existing:
                        existing["gps_location"] = e_gps
                    found = True
                    break
        if not found:
            state["alerts"].insert(0, alert)
